...
module.exports = function serviceSpecsToMongoose (feathersSpec, feathersExtension, depth = 1) {
  ...
  Object.keys(properties).forEach(name => {
    ...
    switch (property.type) {
      case 'array':
        mongooseProperty.type = [typeEquivalence[property.items.type || property.items[0].type]];
        if (property.items && property.items.enum || property.items[0].enum) {
          mongooseProperty.enum = property.items.enum || property.items[0].enum
        }
        break;
      case 'object':
