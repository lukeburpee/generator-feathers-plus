...
module.exports = function specsExpand (specs) {
  ...
  Object.keys(connections).forEach(adapter1 => {
    let { database, adapter, connectionString, customDeps } = connections[adapter1];

    _databases[database] = connectionString || null;
    const { dependencies, connection } = getConnectionInfo(connectionString, database, adapter, customDeps);
    _dbConfigs[database === 'custom' ? adapter : database] = connection;
...
function getConnectionInfo (connectionString, database, adapter, customDeps = '') {
  const sqlPackages = {
    mysql: 'mysql2',
    mssql: 'mssql',
    postgres: 'pg',
  ...
  switch (database) {
    case 'custom':
      dependencies = customDeps.indexOf(',') !== -1 ? customDeps.split(',') : [customDeps]
      return {
        dependencies,
