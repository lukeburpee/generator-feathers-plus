...
async function insertRequiredCustomResources (resources) {
  const resourceKey = `${destinationRoot}/requireCustomResources`;
  code[resourceKey] = {};
  const text = resources.text || [];
  // const otherResourceType = resources.{otherResourceType} || [];
  try {
    for (let i = 0, leni = text.length; i < leni; i++) {
      const textPath = text[i];
      const src = await readFileSync(textPath, 'utf8');
      code[resourceKey][textPath] = src.split(EOL);
    }
  } catch (err) {
    console.log(err);
    throw err;
