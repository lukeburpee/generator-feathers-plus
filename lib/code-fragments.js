  ...
  try {
    for (let i = 0, leni = filePaths.length; i < leni; i++) {
      ...
      if (isCustomResource) {
        ...
        code[`${destinationRoot}/requiredCustomResources`][filePath] = src.split(EOL);
      }
    }
  } catch (err) {
    console.log(err);
...
function flattenCodelistModule (output, module_location, module_code, logger=false) {
  if (module_code === undefined || null) {
    if (logger) {
      logger();
      logger(yellow.bold(`No custom code found in ${module_location}`));
      logger();
      return;
    } else {
      return [];
