...
async function extractProject (specs) {
  const { text } = specs.requiredCustomResources.files;
...
async function extractFiles (filePaths, hasFragments, ifCheck) {
  try {
    for (let i = 0, leni = filePaths.length; i < leni; i++) {
      ...
      if (hasFragments) {
        ...
        code['requiredCustomResources'][filePath] = src.split(EOL);
      }
    }
  } catch (err) {
    console.log(err);
...
function flattenCodelistModule (output, module_location, module_code, logger=false) {
  ...
  Object.keys(module_code).forEach(code_location => {
    let code = module_code[code_location].join('\n');
    if (logger) {
